...
class CustomersRepository {
	...
	async insertAddress(id, address) {
		try {
			const existingAddress = await db('customers_address')
	...
	async listAddresses(id) {
		try {
			const addresses = await db('customers_address').where({
				customers_id: id,
			});
	...
	async update(id, customer) {
		try {
			...
			const updatedCustomer = await db('customers')
				.where({ customers_id: id })
				.update({
					customers_tipo_pessoa: customer.type
						? customer.type
						: existingCustomer.customers_tipo_pessoa,
					customers_email_address: customer.email
						? customer.email
						: existingCustomer.customers_email_address,
					customers_firstname: customer.firstname
						? customer.firstname
						: existingCustomer.customers_firstname,
					customers_lastname: customer.lastname
						? customer.lastname
						: existingCustomer.customers_lastname,
					customers_cpf_cnpj: customer.cpfCnpj
						? customer.cpfCnpj
						: existingCustomer.customers_cpf_cnpj,
					customers_dob: customer.dob
						? customer.dob
						: existingCustomer.customers_dob,
					customers_rg: customer.rg
						? customer.rg
						: existingCustomer.customers_rg,
					customers_ddd1: customer.ddd
						? customer.ddd
						: existingCustomer.customers_ddd1,
					customers_telephone: customer.telephone
						? customer.telephone
						: existingCustomer.customers_telephone,
					customers_gender: customer.gender
						? customer.gender
						: existingCustomer.customers_gender,
				});

			if (customer.address) {
				const updatedAddress = await db('customers_address')
					.update({
					...
					.where({ customers_id: id })
					.where({ address_book_id: customer.address_id });
